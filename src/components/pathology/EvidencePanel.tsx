...
import { Network, ExternalLink, Activity, BookOpen } from 'lucide-react';
...
const EvidenceCard = memo<EvidenceCardProps>(({ item, index }) => (
  <div
    className="bg-white p-3 rounded-xl border border-iris-200 shadow-sm relative group cursor-pointer hover:border-iris-400 transition-all"
  ...
  >
    ...
    <div className="flex justify-between items-start mb-2">
      <span className="text-[9px] font-bold bg-iris-50 text-iris-600 px-2 py-0.5 rounded-full border border-iris-100">
        Top Match: {item.similarity}
      </span>
...
GuideReference.displayName = 'GuideReference';
...
const EvidencePanel: React.FC = () => {
  ...
  const renderEvidenceCard = useCallback(
    (item: EvidenceItem, index: number) => <EvidenceCard key={item.referenceId} item={item} index={index} />,
    []
  );
  ...
  return (
    <aside className="w-full h-full bg-white border-l border-gray-200 flex flex-col z-20 flex-shrink-0 shadow-[-4px_0_20px_rgba(0,0,0,0.02)]">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4 bg-slate-50 no-scrollbar">
        {currentCase.evidence.map(renderEvidenceCard)}
        <GuideReference />
      </div>
    </aside>
  );
